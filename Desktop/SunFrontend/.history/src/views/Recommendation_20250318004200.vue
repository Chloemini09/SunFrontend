  ...
  setup() {
    ...
    const fetchUVData = async () => {
      try {
        ...
        console.log('Request URL:', `http://3.27.122.178:3000/api/uv?lat=${lat}&lng=${lng}`);

        let dataSource = 'API'; // 跟踪数据来源

        try {
          // 尝试获取实际UV数据
          const response = await fetch(`http://3.27.122.178:3000/api/uv?lat=${lat}&lng=${lng}`);
          ...
          // 尝试使用模拟数据
          console.log('Trying mock data URL:', `http://3.27.122.178:3000/api/uv/mock/coords?lat=${lat}&lng=${lng}`);

          try {
            const mockResponse = await fetch(`http://3.27.122.178:3000/api/uv/mock/coords?lat=${lat}&lng=${lng}`);
