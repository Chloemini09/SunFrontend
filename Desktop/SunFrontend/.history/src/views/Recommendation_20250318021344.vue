  ...
  setup() {
    ...
    const fetchUVData = async () => {
      try {
        ...
        console.log('Request URL:', `${apiBaseUrl}/uv?lat=${lat}&lng=${lng}`);

        let dataSource = 'API';

        try {
          // 使用环境变量拼接完整 URL
          const response = await fetch(`${apiBaseUrl}/uv?lat=${lat}&lng=${lng}`);
          ...
          try {
            const mockResponse = await fetch(`${apiBaseUrl}/uv/mock/coords?lat=${lat}&lng=${lng}`);
